...
class Magiv2Processor:
    ...
    def preprocess_inputs_for_crop_embeddings(self, images):
        images = list(images)
        assert isinstance(images[0], np.ndarray)
        return self.crop_embedding_image_preprocessor(
            images, return_tensors="pt"
        ).pixel_values

    # 将 OCR 的生成结果（ID 列表）解码为文本，支持跳过特殊标记。
    def postprocess_ocr_tokens(self, generated_ids, skip_special_tokens=True):
        return self.ocr_preprocessor.batch_decode(
            generated_ids, skip_special_tokens=skip_special_tokens
        )

    # 根据提供的边界框进行裁剪，同时确保边界框有效且大小合适。
        ...
        for scores, labels, bboxes in zip(batch_scores, batch_labels, batch_bboxes):
            ...
            for ps, pb, pl, pi in zip(scores, bboxes, labels, indices):
                ...
                if ps < panel_detection_threshold:
                    continue
                if (
                    union_of_panels_so_far.intersection(panel_polygon).area
                    / panel_polygon.area
                    > 0.5
                ):
                    continue
    ...
    def _get_indices_of_texts_to_keep(
        ...
        for scores, labels, bboxes in zip(batch_scores, batch_labels, batch_bboxes):
            indices = torch.where((labels == 1) & (scores > text_detection_threshold))[
                0
            ]
            bboxes = bboxes[indices]
            scores = scores[indices]
            ...
            for ts, tb, tl, ti in zip(scores, bboxes, labels, indices):
                ...
                for t in texts_to_keep_as_shapely_objects:
                    if (
                        t.intersection(text_polygon).area / t.union(text_polygon).area
                        > 0.5
                    ):
                        should_append = False
                        break
    ...
    # 类似于 _get_indices_of_texts_to_keep，但用于筛选尾部（标签为 3 的特定文本特征）。
    def _get_indices_of_tails_to_keep(
        self, batch_scores, batch_labels, batch_bboxes, text_detection_threshold
    ):
        indices_of_texts_to_keep = []
        for scores, labels, bboxes in zip(batch_scores, batch_labels, batch_bboxes):
            indices = torch.where((labels == 3) & (scores > text_detection_threshold))[
                0
            ]
            bboxes = bboxes[indices]
            scores = scores[indices]
    ...
    def _convert_annotations_to_coco_format(self, annotations):
        ...
        for annotation in annotations:
            ...
            for bbox, label in zip(
                annotation["bboxes_as_x1y1x2y2"], annotation["labels"]
            ):
